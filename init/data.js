...
const optimizedListings = sampleListings.map((listing) => {
  const imageUrl = new URL(listing.image);

  Object.entries(imageParams).forEach(([key, value]) => {
    imageUrl.searchParams.set(key, value);
  });

  return { ...listing, image: imageUrl.toString() };
});

module.exports = { data: optimizedListings };
